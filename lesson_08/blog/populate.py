...
with app.app_context():
    ...
    x = [(p, u) for p in range(1, 6) for u in sample(range(10, 15), randint(0, 3))]
    likes = [Like(post_id=p, user_id=u)
             for p in range(1, len(posts))
             for u in sample(range(1, len(users)), randint(0, 3))]
